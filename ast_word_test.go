...
func TestParameterExpansion(t *testing.T) {
	doTests(t, []sourceFn{
		...
		{"${a:1:2}", func(t *test, tk Tokens) { // 21
			t.Output = ParameterExpansion{
				Type: ParameterSubstring,
				Parameter: Parameter{
					Parameter: &tk[1],
		...
		{"${a:1:2b}", func(t *test, tk Tokens) { // 54
			t.Err = Error{
